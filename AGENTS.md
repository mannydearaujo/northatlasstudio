...
For audits, use `agency-skills/audit-site/assets/report-template.html`.
For weekly reports, use `agency-skills/report-to-client/assets/report-template.html`.
...
customers — don't force "we" onto a client's authentic voice. `de-ai-copy` checks for this
...
Added 2026-07-23, prompted by reviewing how other AI-native operators (e.g. Ryan Carson/Untangle)
run agent fleets: never give an agent standing production write access to any credential — North
Atlas's own or a client's. This already holds for North Atlas's own tooling (the GA4/Search Console
service-account key lives at `GA4_SERVICE_ACCOUNT_KEY_PATH` in `~/.secrets/`, outside the repo;
`PAGESPEED_API_KEY` lives only in `.env`/`.env.local`, never committed or printed) — this section
makes the underlying principle explicit and extends it forward to client automation work.

- **No agent, skill, or automation holds standing write credentials to a client's tools** (MoeGo,
  Jobber, QuickBooks, Square, Formspree, a client's CMS/host, etc.). Credentials are requested and
  used per task, scoped to the minimum access needed, the same way `.env`/`~/.secrets/` already keep
  North Atlas's own keys out of the repo and out of any agent's default context.
- This applies starting now, in planning, even though Phase 4 (n8n Cloud Starter) and real
  client-facing automation builds are still deferred — it's cheap to state before any client
  credential is ever involved, and expensive to retrofit after one is.
- When Phase 4 work actually starts, the access workflow in "Measurement Rules" below (minimum
  permissions, verify ownership, don't claim historical data without access) extends to any new
  automation credential the same way it already governs GA4/GTM/Search Console access.
...
- **`north-atlas-pipeline-smoke-test`** (added 2026-07-23, prompted by reviewing Ryan Carson/Untangle's end-to-end signup-test pattern): a daily health check for this whole chain — submits a clearly-marked, harmless synthetic submission through the real Formspree form, then confirms on a later run that it was captured and correctly declined as test data by `north-atlas-audit-lead-autodraft`'s existing guardrail. Catches silent breakage (a broken Gmail filter, a dead webhook) before it costs a real lead, without ever risking a real audit running against an unrelated business. Not yet scheduled via launchd — see `TASKS.md`.
...
2. Send the highest-fit prospects through `audit-site` and `google-business-profile-optimizer`.
...
Keep the shared `agency-skills/` source authoritative, then mirror skills into Codex and Claude when they change. The existing `audit-site` skill was ported into the shared library and Codex; do not rebuild it from scratch.
...
- North Atlas Studio's own operational inbox (capture/curate step of the AI-native loop): use `inbox-pull` to pull new items into the vault, then `inbox-file` to file/flag them. See the "AI-Native Operating Loop" section above for current status.
...
- A captured Free Site Audit form submission (Phase 2 of the AI-native loop): `inbox-file` hands this to `north-atlas-audit-lead-autodraft` automatically — it is not normally invoked directly. `north-atlas-audit-lead-watch` also catches these hourly, independent of the daily sweep.
- Checking whether the Free Site Audit lead pipeline itself is still working end to end, or running the scheduled daily pipeline health-check job: use `north-atlas-pipeline-smoke-test`.
- Scraped/Apify/Maps lead lists: use `apify-local-lead-prospecting`.
- Existing website audit or Free Site Audit deliverable: use `audit-site`.
- Running or following up on a free AI Operations Check call (Core Service #4's discovery call), or analyzing a business-workflow discovery-call transcript to scope/price an AI Operations install: use `audit-operations`. Internal diagnostic only — hands off to `offer-sheet-builder` for anything client-facing, same split as `audit-site`.
...
- Writing, rewriting, or optimizing an SEO page or blog post (not just planning or auditing one): use `seo-content-writer` — it hands off to `city-service-cluster-builder`/`localized-blog-opportunity-finder` for planning first and to `check-schema` afterward. Use `alpha-seo-content` instead when the client is Alpha Seamless Gutter specifically.
- Launch QA: use `launch-check`, `check-schema`, and `lighthouse-technical-seo-fixer`.
- Retainers: use `report-to-client`, `gbp-posting-calendar`, `compliant-review-engine`, and `localized-blog-opportunity-finder`.
- Proof, case studies, and proposals: use `case-study-and-proof-builder` and `offer-sheet-builder`.
- Any client-facing copy before it ships (pages, blogs, GBP posts, outreach, offer sheets, case studies): run `de-ai-copy` as the standard pre-publish pass after `seo-content-writer` or any content generation.
...
- Complete JSON-LD design beyond basic LocalBusiness/FAQPage (Service, Offer, Organization, reviews, entity linking): use `write-schema`; keep `check-schema` for auditing what already exists.
